...
- **MCP auth (two modes, profile-based):**
  - `mcp-oauth2` profile: OAuth 2.1 with Dynamic Client Registration (DCR). Enables native Claude connector support on all platforms (desktop, web, mobile) without `mcp-remote`. Uses `mcp-authorization-server` and `mcp-server-security` from `org.springaicommunity:mcp-security:0.1.5`.
