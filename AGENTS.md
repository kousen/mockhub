...
- **Two ChatClient beans in `AiConfig`:** `chatClient` (system prompt + memory advisor + tools, for `ChatService`) and `plainChatClient` (bare, for one-shot calls in `PricePredictionService` and `RecommendationService`). Since Spring AI 2.0 GA, `MessageChatMemoryAdvisor` requires a conversation id on every call — one-shot services must use the plain client or they fail with 400 "conversationId cannot be null". Inject with `@Qualifier("plainChatClient")`.
...
- **Circular dependency:** with `mockhub.mcp.enabled=true` (production), `anthropicChatModel` → `toolCallingManager` → `toolCallbackResolver` → `mcpToolCallbackProvider` → `PricingTools` → `PricePredictionService` → *any* `ChatClient` → `anthropicChatModel` forms a cycle — every ChatClient bean needs the model, so switching to `plainChatClient` does NOT remove it. Broken with `@Lazy` on the `ChatClient` parameter in `PricePredictionService`. The cycle does not form when MCP is disabled (the test default), so `AiEnabledContextIntegrationTest` boots the context with `mockhub.mcp.enabled=true` to catch regressions — removing that `@Lazy` crashed production once (#291/#292).
...
- **`DemoAccountSeeder`** — no profile restriction, runs on every startup in all environments (kill switch: `DEMO_SEED_ACCOUNTS=false`). Seeds the course demo accounts `alice@mockhub.com` / `bob@mockhub.com` (deliberately public teaching credentials on a mock marketplace; drift reverted on startup) plus up to three confirmed orders of history for Bob, tagged with `demo-seed-*` idempotency keys. `DemoResetService` skips those orders, so `POST /api/v1/admin/demo/reset` returns a demo account to the seeded baseline instead of zero. Runs after `DataSeeder` (`@Order(10)` vs `@Order(20)`) so dev-profile listings exist first.
...
- **MCP server** — 32 tools registered (EventTools, PricingTools, CartTools, OrderTools, MandateTools, AgentApprovalTools, PaymentCredentialTools, AgentRiskTools, AgentPurchaseEvidenceTools) via `spring-ai-starter-mcp-server-webmvc`. Uses Streamable HTTP transport (protocol: `STREAMABLE`) at `/mcp`.
...
- **Agent purchase approvals** — human-in-the-loop checkpoint in `com.mockhub.agentapproval`. An agent calls `proposePurchase` (MCP) to create a PROPOSED approval snapshot; the user approves/denies **on the website only** (`/my/approvals`, `ApprovalsPage`) — the `approvePurchase`/`denyPurchase` MCP tools were deliberately removed because an agent acting as the pinned OAuth user could approve its own proposal in-band, defeating the checkpoint; `checkout`/`completeCheckout` accept an optional `approvalId` that must match the agent, mandate, and total. Used when a mandate's `approvalMode` is `APPROVAL_REQUIRED`. Statuses: PROPOSED → APPROVED/DENIED → COMPLETED/FAILED/EXPIRED. Expiry transition is handled durably by `LifecycleCleanupService` (the read-path check is timestamp-based, so rejection holds regardless of stored status).
