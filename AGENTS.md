...
- **Circular dependency (historical):** MCP tool registration → PricingTools → PricePredictionService → ChatClient used to create a cycle, broken with `@Lazy`. Since `PricePredictionService` moved to `plainChatClient` (which has no tools), the cycle no longer exists and the `@Lazy` was removed.
