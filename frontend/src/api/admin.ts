...
export async function getAdminEvents(
  ...
  const response = await apiClient.get<PageResponse<AdminEvent>>(
    '/admin/events',
    { params: { page, size } },
  );
  return response.data;
...
export async function updateEvent(
  ...
  const response = await apiClient.put<AdminEvent>(
...
export async function updateUserStatus(data: UserStatusUpdate): Promise<AdminUser> {
  const response = await apiClient.patch<AdminUser>(
    `/admin/users/${data.userId}/status`,
...
export async function getAdminOrders(
  ...
  const response = await apiClient.get<PageResponse<AdminOrderSummary>>(
    '/admin/orders',
    { params: { page, size } },
  );
  return response.data;
