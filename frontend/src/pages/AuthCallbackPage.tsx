...
function getInitialError(searchParams: URLSearchParams): string | null {
  ...
  if (errorParam) {
    return errorParam === 'oauth_no_email'
...
export function AuthCallbackPage() {
  ...
  useEffect(() => {
    if (initialError || !code || exchanged.current) {
      return;
    }
    ...
    exchangeOAuth2Code(code)
      ...
      .catch(() => {
        setExchangeError(
          'Failed to complete sign-in. The code may have expired. Please try again.',
        );
      });
  }, [code, initialError, login, navigate]);
